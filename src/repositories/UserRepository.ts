...
export class UserRepository extends BaseRepository<IUser> {
  ...
  async updatePassword(userId: string | any, hashedPassword: string): Promise<IUser | null> {
    // Handle both userId and email
    const filter = userId.includes('@') ? { email: userId.toLowerCase() } : { _id: userId };
  ...
  async updateStatus(userId: string, status: 'active' | 'inactive' | 'suspended'): Promise<IUser | null> {
