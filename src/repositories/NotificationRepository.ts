...
export class NotificationRepository extends BaseRepository<INotification> {
  ...
  async markAsRead(notificationId: string): Promise<INotification | null> {
    return this.model
      .findByIdAndUpdate(notificationId, { readAt: new Date() }, { new: true })
      .exec();
  }

  async markAllAsRead(notifiableId: string | Types.ObjectId): Promise<void> {
    await this.model
      .updateMany({ notifiableId, readAt: null }, { readAt: new Date() })
      .exec();
  }
  ...
  async deleteMany(notifiableId: string | Types.ObjectId): Promise<void> {
