...
adminSchema.index({ department: 1, adminLevel: 1 });
...
adminSchema.pre("save", function (this: IAdmin, next) {
    if ((this as any).isNew && this.permissions.length === 0) {
        switch (this.adminLevel) {
            case "super_admin":
                this.permissions = Object.values(ADMIN_PERMISSIONS).flatMap(
                    (category) => Object.values(category)
                );
                break;
            case "admin":
                this.permissions = [
                    ...
                    ...Object.values(ADMIN_PERMISSIONS.VENDORS),
                    ADMIN_PERMISSIONS.SYSTEM.ANALYTICS,
                ];
                break;
            case "moderator":
                this.permissions = [
                    ...
                    ADMIN_PERMISSIONS.VENDORS.VIEW,
                    ADMIN_PERMISSIONS.VENDORS.UPDATE,
                ];
                break;
        }
    }
